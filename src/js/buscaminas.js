...
// Función independiente para dibujar el DOM basándome en 'board'
function renderBoard() {
    ...
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            ...
            // Si cargo partida, aplico las clases visuales
...
// Guardo la partida transformando el objeto a JSON
function saveGame() {
...
function loadGame() {
    ...
    if (saved) {
        ...
        // Actualizo la vista de inputs y reloj
...
function revealCell(row, col) {
    ...
    // Le añado la clase CSS revealed
...
function gameOver() {
    ...
    // Recorro matriz para mostrar dónde estaban ocultas las demás minas
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            if (board[i][j].isMine) {
